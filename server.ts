...
import { pdrVectorDbInstance } from "./server_rag";
...
app.post("/api/chat", async (req, res) => {
  try {
    ...
    // Initialize/Ensure embeddings if the key is available
    if (apiKey) {
      try {
    ...
    // Get last user question to perform RAG retrieval search
...
`;
    ...
    if (!apiKey) {
      console.log("[server] GEMINI_API_KEY is missing. Returning RAG lexical search fallback response.");
...
_Для повного доступу до розумного AI-помічника з роз'ясненнями, будь ласка, додайте свій **GEMINI_API_KEY** у налаштуваннях (Secrets) вашого робочого простору._`;
...
async function startServer() {
  // Vite middleware for development only (dynamic import avoids bundling vite into Vercel serverless)
...
// Only start the server listener if not running in Vercel Serverless environment
